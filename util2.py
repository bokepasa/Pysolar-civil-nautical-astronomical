...
@check_aware_dt('when')
def get_sunrise_sunset_transit(latitude_deg, longitude_deg, when, SUN_ALTITUDE_TWILIGHT):
    ...
    >>> sr, ss, tr = sb.get_sunrise_sunset_transit(lat, lon, now, twilight)
    >>> print('sunrise: ', sr)
    >>> print('sunset:', ss)
    >>> print('transit:', tr)
    ...
    if (SUN_ALTITUDE_TWILIGHT==0):
        SUN_ALTITUDE_TWILIGHT = 90.833
    if (SUN_ALTITUDE_TWILIGHT==1):
        SUN_ALTITUDE_TWILIGHT = 96  
    if (SUN_ALTITUDE_TWILIGHT==2):
        SUN_ALTITUDE_TWILIGHT = 102
    if (SUN_ALTITUDE_TWILIGHT==3):
        SUN_ALTITUDE_TWILIGHT = 108
    ...
    ha = \
        (
            math.acos(
                math.cos(math.radians(SUN_ALTITUDE_TWILIGHT))
            /
                (
